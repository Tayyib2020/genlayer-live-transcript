...
export async function refreshSessionVerification(sessionId, services = {}) {
  ...
  const current = await getSessionVerification(sessionId);
  ...
  try {
    ...
    return getSessionVerification(sessionId);
  }
  ...
  if (transactionState.kind === "pending") return getSessionVerification(sessionId);
...
export async function submitSessionVerification(sessionId, services = {}) {
  ...
  if (prepare.kind === "existing") {
    if (prepare.verification.transactionHash) {
      return { kind: "existing", verification: await refreshSessionVerification(sessionId, services) };
    }
  ...
  return { kind: "pending", verification: await refreshSessionVerification(sessionId, services) };
}
