...
function mapVerification(row) {
  ...
  return {
    ...
    outcomeProcessingPending: row.verification_status === "pending" && row.transaction_status === "FINALIZED_PROCESSING_OUTCOME",
    finalizationPending: row.verification_status === "pending" && ["ACCEPTED", "FINALIZED_PROCESSING_OUTCOME"].includes(row.transaction_status),
    reason: row.reason,
